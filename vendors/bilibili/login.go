...
func LoginWithQRCode(ctx context.Context, key string) (bilibili.QRCodeStatus, *http.Cookie, error) {
	...
	if loginQRResp.Code != 0 {
		return 0, nil, errors.New(loginQRResp.Message)
	}
	...
	case qrStatusScanned:
		return bilibili.QRCodeStatus_SCANNED, nil, nil
	case qrStatusNotScanned:
		return bilibili.QRCodeStatus_NOTSCANNED, nil, nil
	default:
		return 0, nil, fmt.Errorf("unknown qr code status: %d", loginQRResp.Data.Code)
	}
}
